...
struct vma_head *vma_create(void) {
    struct vma_head *h = (struct vma_head *)VMA_VIRTUAL(mmu_alloc(1));
    mmu_map(h, VMA_PHYSICAL(h), PTE_PRESENT | PTE_WRITABLE | PTE_USER);
    h->head = (struct vma_block *)VMA_VIRTUAL(mmu_alloc(1));
    mmu_map(h->head, VMA_PHYSICAL(h->head), PTE_PRESENT | PTE_WRITABLE | PTE_USER);
    h->head->next = h->head;
    h->head->prev = h->head;
    h->head->size = 0;
    h->head->checksum = 0;
...
void vma_destroy(struct vma_head *h) {
    ...
    while (current != h->head) {
        ...
        mmu_unmap(current);
        mmu_free(VMA_PHYSICAL(current), 1);
        current = next;
    }

    mmu_free(VMA_PHYSICAL(h->head), 1);
    mmu_unmap(h->head);
    mmu_free(VMA_PHYSICAL(h), 1);
    mmu_unmap(h);
}

void *vma_map(struct vma_head *h, uint64_t pages, uint64_t phys, uint64_t virt, uint64_t flags) {
    struct vma_block *block = (struct vma_block *)VMA_VIRTUAL(mmu_alloc(1));
    mmu_map(block, VMA_PHYSICAL(block), PTE_PRESENT | PTE_WRITABLE);
    block->next = h->head;
